...
int main(int argc, char *argv[]) {
  if (argc < 3) {
    printf("\033[31marguments required:\033[0m executable_name and files...\nexample: foundation hello hello.c\n");
    exit(1);
  }
  ...
  printf("Attempting to open file 'build.cmd'");
  FILE *outfile;
  if (fopen_s(&outfile, "build.cmd", "w") != 0) {
    printf(",\033[31m failed.\033[0m\n");
    exit(1);
  }
  printf(", done.\n");
  ...
  if (argc == 3) {
    // Single file, skipping compilation to object file
    printf("Generating line for %s -> %s.exe", argv[2], exec_name);
    fprintf_s(outfile, "echo (@) - Compiling \033[33m%s\033[0m -^> %s.exe\n",
              argv[2], exec_name);
    fprintf_s(outfile, "gcc %s -o %s %s\n",
              argv[2], exec_name, standard_opts);
    printf(", done.\n");
  } else {
    // Multiple files, compile to object and link
    char *objn[argc-2];
    for (int i = 2; i < argc; i++) {
      printf("Generating line for %s -> %s", argv[i], dotc_doto(argv[i]));
      fprintf_s(outfile, "echo (%i) - Compiling \033[33m%s\033[0m -^> %s\n",
          i - 2, argv[i], dotc_doto(argv[i]));
      fprintf_s(outfile, "gcc %s -c %s\n",
                standard_opts, argv[i]);
      objn[i - 2] = dotc_doto(argv[i]);
      printf(", done.\n");
    }
    printf("Generating linkage line -> %s.exe", exec_name);
    fprintf_s(outfile, "echo (@) - Linking \033[33m");
